...
enum PhonePinnedSyncStatus: Equatable, Sendable {
    case disabled
    case pending
    case synced
...
enum PhonePinnedSyncStartError: Error {
    case accountUnavailable
}

enum PhonePinnedSyncTransportError: Error {
    case accountUnavailable
}

struct PhoneCloudAccountContext: @unchecked Sendable {
    let accountIdentity: String
    let container: CKContainer?
}

protocol PhoneCloudKitSession: Sendable {
...
private final class PhoneCKSyncSession: PhoneCloudKitSession, @unchecked Sendable {
    private let engine: CKSyncEngine
    private let delegate: PhoneCloudKitSyncDelegate

    init(engine: CKSyncEngine, delegate: PhoneCloudKitSyncDelegate) {
        self.engine = engine
        self.delegate = delegate
    }

    func prepareZone() async throws {
        engine.state.add(pendingDatabaseChanges: [.saveZone(CKRecordZone(zoneID: PhoneCloudRecordCodec.zoneID))])
        try await engine.sendChanges(.init(scope: .zoneIDs([PhoneCloudRecordCodec.zoneID])))
    }

    func fetch() async throws {
        try await engine.fetchChanges(.init(scope: .zoneIDs([PhoneCloudRecordCodec.zoneID])))
    }

    func send(_ mutations: [PinnedMutation]) async throws {
        await delegate.replacePending(mutations)
        engine.state.add(pendingRecordZoneChanges: mutations.map {
...
actor PhoneCloudKitTransport: PhonePinnedSyncTransport {
    typealias ResolveAccount = @Sendable () async throws -> PhoneCloudAccountContext
    typealias MakeSession = @Sendable (PhoneCloudAccountContext) async throws -> any PhoneCloudKitSession
    ...
    init(
        ...
        resolveAccount = {
            ...
            do {
                ...
                throw PhonePinnedSyncStartError.accountUnavailable
            }
            guard accountStatus == .available else { throw PhonePinnedSyncStartError.accountUnavailable }
            do {
                return try PhoneCloudAccountContext(
        ...
        makeSession = { context in
            guard let container = context.container else { throw PhonePinnedSyncStartError.accountUnavailable }
            ...
            return PhoneCKSyncSession(engine: CKSyncEngine(configuration), delegate: delegate)
        }
    }
    ...
    private static func mapTransportError(_ error: Error) -> Error {
        ...
        case .notAuthenticated:
            return PhonePinnedSyncTransportError.accountUnavailable
        default:
            return error
        }
    }
    ...
    func handleEvent(_ event: CKSyncEngine.Event, syncEngine _: CKSyncEngine) async {
        ...
        case let .stateUpdate(update):
            do {
                ...
                try await eventHandler(.stateUpdate(JSONEncoder().encode(update.stateSerialization)))
            } catch { await eventHandler(.retryableFailure) }
...
enum PhonePinnedSyncEvent: Sendable {
...
protocol PhonePinnedSyncTransport: Sendable {
    func start(eventHandler: @escaping @Sendable (PhonePinnedSyncEvent) async -> Void) async throws
    func fetch() async throws
    func send(_ mutations: [PinnedMutation]) async throws
    func cancel() async
    /// Drops the session without cancelling it, for the one path that runs inside a
    /// CKSyncEngine event callback. See `PhonePinnedSyncEngine.stopTransport(with:insideEventCallback:)`.
    func releaseWithoutCancelling() async
}

actor PhonePinnedSyncEngine {
    private let makeTransport: @Sendable () throws -> any PhonePinnedSyncTransport
    private let pendingMutations: @Sendable () async throws -> [PinnedMutation]
    private let acknowledge: @Sendable ([UUID]) async throws -> Void
    private var applyRemote: @Sendable (PinnedMutation) async throws -> Void
    private let persistState: @Sendable (Data) async throws -> Void
    ...
    private var transport: (any PhonePinnedSyncTransport)?
    ...
    private var refreshSucceeded: (@Sendable () async throws -> Void)?
    ...
    private(set) var status: PhonePinnedSyncStatus = .disabled

    init(
        makeTransport: @escaping @Sendable () throws -> any PhonePinnedSyncTransport,
        pendingMutations: @escaping @Sendable () async throws -> [PinnedMutation] = { [] },
        acknowledge: @escaping @Sendable ([UUID]) async throws -> Void = { _ in },
        applyRemote: @escaping @Sendable (PinnedMutation) async throws -> Void = { _ in },
        persistState: @escaping @Sendable (Data) async throws -> Void = { _ in },
    ...
    func installRemoteApply(_ applyRemote: @escaping @Sendable (PinnedMutation) async throws -> Void) {
        self.applyRemote = applyRemote
    }

    func installStatusObserver(_ observer: @escaping @Sendable (PhonePinnedSyncStatus) async -> Void) async {
        statusObserver = observer
        await observer(status)
    }

    func installRefreshSucceeded(_ handler: @escaping @Sendable () async throws -> Void) {
        refreshSucceeded = handler
    }
    ...
    private func startEnabledSession(recoveryIncidentEpoch allowedIncident: UInt64? = nil) async throws {
        ...
        do {
            ...
            try await newTransport.start { [weak self] event in await self?.handle(event, epoch: epoch) }
            didStart = true
            ...
            if Self.isAccountUnavailable(error) {
                ...
                await newTransport.cancel()
            } else if error is PhoneSyncStateStoreError {
                transport = nil
    ...
    private func handle(_ event: PhonePinnedSyncEvent, epoch: UInt64) async {
        guard epoch == sessionEpoch, transport != nil else { return }
        do {
            switch event {
            case let .stateUpdate(data): try await withEventLease { try await persistState(data) }
            ...
            case .fetchCompleted: await fetchCompleted(epoch: epoch)
            case let .sent(ids):
    ...
    private static func isAccountUnavailable(_ error: Error) -> Bool {
        if error is PhonePinnedSyncStartError || error is PhonePinnedSyncTransportError {
            return true
        }
        return (error as? CKError)?.code == .notAuthenticated
    }

    private func updateStatus(_ newStatus: PhonePinnedSyncStatus) async {
        status = newStatus
